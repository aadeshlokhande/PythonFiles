...
b1=ttk.Button(frame, command=lambda aurl= url7:OpenUrl(aurl)) # type: ignore
b1.pack(side=LEFT)
m1=PhotoImage(file="firefox.png")
b1.config(image=m1)
tm1=m1.subsample(3,3)
b1.config(image=tm1)
b2=ttk.Button(frame, command=lambda aurl=url8:OpenUrl(aurl)) # type: ignore
b2.pack(side=LEFT)
m2=PhotoImage(file="bing.png")
b2.config(image=m2)
tm2=m2.subsample(6,6)
b2.config(image=tm2)
b3=ttk.Button(frame, command=lambda aurl=url9:OpenUrl(aurl)) # type: ignore
b3.pack(side=LEFT)
...
subMenu.add_command(label="Gmail", command=lambda aurl=url1:OpenUrl(aurl)) # type: ignore
subMenu.add_command(label="Y! Mail", command=lambda aurl=url2:OpenUrl(aurl)) # type: ignore
subMenu.add_command(label="Youtube", command=lambda aurl=url3:OpenUrl(aurl)) # type: ignore
subMenu.add_command(label="Facebook", command=lambda aurl=url4:OpenUrl(aurl)) # type: ignore
subMenu.add_command(label="Github", command=lambda aurl=url5:OpenUrl(aurl)) # type: ignore
subMenu.add_command(label="LinkedIn", command=lambda aurl=url6:OpenUrl(aurl)) # type: ignore
